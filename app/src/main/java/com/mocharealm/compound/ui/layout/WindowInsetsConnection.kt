...
fun Modifier.imeNestedScroll(focusRequester: FocusRequester): Modifier {
...
/**
 * Returns a [NestedScrollConnection] that can be used with [androidx.compose.foundation.layout.WindowInsets] on Android
 * [R][Build.VERSION_CODES.R] and later.
 *
 * The [NestedScrollConnection] can be used when a developer wants to control a [androidx.compose.foundation.layout.WindowInsets],
 * either directly animating it or allowing the user to manually manipulate it. User interactions
 * will result in the [androidx.compose.foundation.layout.WindowInsets] animating either hidden or shown, depending on its current
 ...
 *   [WindowInsetsSides.Companion.Left], [WindowInsetsSides.Companion.Top], [WindowInsetsSides.Companion.Right],
 *   [WindowInsetsSides.Companion.Bottom], [WindowInsetsSides.Companion.Start], [WindowInsetsSides.Companion.End].
 */
@ExperimentalLayoutApi
@Composable
...
private class WindowInsetsNestedScrollConnection(
    ...
    private fun scroll(available: Offset, scrollAmount: Float): Offset {
        ...
        if (animationController == null) {
            if (scrollAmount > 0f && !windowInsets.isVisible) {
    ...
    private fun adjustInsets(inset: Float) {
        animationController?.let { controller ->
            ...
            val nextIntInsets = sideCalculator.adjustInsets(currentInsets, inset.fastRoundToInt())
    ...
    override fun onReady(controller: WindowInsetsAnimationController, types: Int) {
        ...
        holder.ime.insets = androidx.core.graphics.Insets.toCompatInsets(controller.shownStateInsets)
    ...
    companion object {
        ...
        fun chooseCalculator(side: WindowInsetsSides, layoutDirection: LayoutDirection) =
            when (side) {
                ...
                WindowInsetsSides.Companion.End ->
                    if (layoutDirection == LayoutDirection.Ltr) {
                        RightSideCalculator
                    } else {
                        LeftSideCalculator
